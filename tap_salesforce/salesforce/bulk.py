...
class Bulk:
    ...
    def _bulk_query_with_pk_chunking(self, catalog_entry, start_date):
        ...
        if batch_status["failed"]:
            raise TapSalesforceExceptionError("One or more batches failed during PK chunked job")
    ...
    def _create_job(self, catalog_entry, pk_chunking=False):
        ...
        if pk_chunking:
            ...
            headers["Sforce-Enable-PKChunking"] = f"true; chunkSize={DEFAULT_CHUNK_SIZE}"
            ...
            if any(catalog_entry["stream"].endswith(suffix) for suffix in ["CleanInfo", "History"]):
                ...
                headers["Sforce-Enable-PKChunking"] = headers["Sforce-Enable-PKChunking"] + f"; parent={parent}"
    ...
    def _get_batches(self, job_id):
        ...
        batches = xmltodict.parse(resp.text, xml_attribs=False, force_list=("batchInfo",))["batchInfoList"]["batchInfo"]
    ...
    def get_batch_results(self, job_id, batch_id, catalog_entry):
        ...
        batch_result_list = xmltodict.parse(batch_result_resp.text, xml_attribs=False, force_list={"result"})[
            "result-list"
        ]
